...
class HYG_Map():
    ...
    def space_bubble(self, distance):
        ...
        trace = go.Scatter3d(x=X, y=Y, z=Z, mode='markers', marker=Markers, text=names)
        layout = go.Layout(
            scene=dict(
                xaxis=dict(nticks=6, range=[-distance, distance], backgroundcolor="rgb(0,0,0)"),
       ...
       # width=800, height=600,  # Set static size or use relative units
        margin=dict(r=20, l=10, b=10, t=10),
        )
    
        print("Plotting Space Bubble...")
        print(f"{len(names)} stars have been plotted.")
        
        return fig  # Return the figure object instead of writing to file
...
@app.route('/hr-diagram')
def hr_diagram():
    fig = hyg_map.Hertzprung_Russell(20000)
    plot_json = pio.to_json(fig)
    return render_template('hr_diagram.html', plot_json=plot_json)

@app.route('/space-bubble')
def space_bubble():
    fig = hyg_map.space_bubble(3500)
    plot_json = json.loads(pio.to_json(fig))
    return render_template('space_bubble.html', plot_json=plot_json)
